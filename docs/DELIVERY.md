...
The default encoded-message budget is 15 MiB, including MIME headers and base64 transfer encoding. Attachments are prioritized as presentation, release, supplement, current 10-Q/10-K, then other source documents. Packaging counts the exact SMTP encoding without retaining trial-message buffers and splits oversized packets into numbered messages. A document that cannot fit by itself is omitted from the attachment and listed with its authoritative source URL and local archive path. The final serialized message is checked again; `deliver_messages` refuses any `.eml` that still exceeds the configured budget before opening SMTP.

Install full monitoring with `uv sync --extra sec` before configuring a scheduled run. Keep `--extra sec` on later `uv run` commands so synchronization retains EdgarTools. The offline doctor and scheduler installer check that the provider is installed without importing it or making a network request. The smaller base installation supports archived report rendering, packaging and delivery; it does not satisfy full-monitoring readiness.
