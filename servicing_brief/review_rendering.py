...
from .company_boundary import canonical_event, normalize_cik


REVIEW_TEMPLATE_NAME = "review_email.html.j2"
LEGACY_TEMPLATE_NAME = "email-template.html.j2"


def create_review_environment(template_root: str | Path | None = None) -> Environment:
    """Create the Jinja environment used by the review renderer.

    The package template is the default so the renderer works from an
    installed wheel.  A checkout-local template root remains supported for
    legacy callers and takes precedence over the package templates.
    """
    package_loader = PackageLoader("servicing_brief", "templates")
    if template_root is None:
        loader = package_loader
        template_name = REVIEW_TEMPLATE_NAME
    else:
        loader = FileSystemLoader([Path(template_root).resolve()])
        # The legacy template is an include shim.  PackageLoader supplies the
        # canonical template and imported package partials when it is used.
        from jinja2 import ChoiceLoader

        loader = ChoiceLoader([loader, package_loader])
        template_name = (
            LEGACY_TEMPLATE_NAME
            if (Path(template_root).resolve() / LEGACY_TEMPLATE_NAME).is_file()
            else REVIEW_TEMPLATE_NAME
        )
    env = Environment(loader=loader, autoescape=True, trim_blocks=True, lstrip_blocks=True)
    env._review_template_name = template_name  # type: ignore[attr-defined]
    return env
...
def _render_html(env: Environment, companies: list[dict[str, Any]], *, combined: bool, universe_note: str, attachment_note: str, full_document: bool) -> str:
    ...
    template_name = getattr(env, "_review_template_name", None)
    if template_name:
        template = env.get_template(template_name)
    else:
        # Existing callers may provide their own environment with the legacy
        # template name.  Prefer it when available, then use the package name.
        try:
            template = env.get_template(LEGACY_TEMPLATE_NAME)
        except TemplateNotFound:
            template = env.get_template(REVIEW_TEMPLATE_NAME)
...
def main(
    ...
    *,
    default_review_root: str | Path | None = None,
    ...
    if default_review_root is None and args.review_root is None:
        raise ValueError("--review-root is required when using the packaged renderer")

    script_dir = Path(default_review_root).resolve() if default_review_root is not None else Path(__file__).resolve().parent
    review_root = (args.review_root or script_dir).resolve()
    output_default = Path(default_output_dir).resolve() if default_output_dir is not None else review_root
    output_dir = (args.output_dir or output_default).resolve()
    ...
    env = create_review_environment(template_root)
